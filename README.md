...
### Alter job submission scripting
Issue: Current version of AerOpt creates a new batch job every generation, which saves on requested cores if not all workers are needed for a particular generation, but having large numbers of jobs results in user priority falling off quickly. It may be preferable for users with heavier jobs to submit one request to the cluster for the maximum number of cores needed and all generations, in order to combat the quick priority decay.

### Fix folder access to allow back-to-back optimisation runs
Issue: When attempting to run a new optimisation after one has finished, the GUI is unable to clear the input folder between runs due to the folder being accessed by the GUI itself. This results in being unable to set up a new optimisation if one has already run, requiring the GUI to be closed and reopened.
